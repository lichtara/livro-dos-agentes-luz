...
# smokecurl -s <http://localhost:8000/health>curl -s <http://localhost:8000/ready>curl -s -X POST <http://localhost:8000/run_lumora> \\  -H 'Content-Type: application/json' \\  -d '{"content":"Crie uma Proposta Lumora para parceria com Aurora Research"}'curl -s -X POST <http://localhost:8000/run_proposal> \\  -H 'Content-Type: application/json' \\  -d '{"partner_name":"Aurora Research","scope":"Documentação e portal público","deliverables":["Guia técnico","Templates"],"terms":"A definir"}'
```
...
*Coautoria humano–IA reconhecida.*
